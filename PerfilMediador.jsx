...
export default function PerfilMediador() {
  ...
  async function saveProfile(e){
    ...
    } catch(e){ setMsg("❌ " + (e.message || "Error guardando perfil")); }
  }
  ...
  async function uploadFile(kind, file){
    ...
    try {
      ...
      fd.append("file", file);
      const r = await fetch("/api/upload/file", { // usa tu route de subida local
        method: "POST",
        body: fd
      });
      const data = await r.json();
    ...
    } catch(e){ setMsg("❌ " + (e.message || "Error subiendo archivo")); }
  }
  ...
  async function changePassword(e){
    ...
    } catch(e){ setMsg("❌ " + (e.message || "Error cambiando contraseña")); }
  }

  return (
    <>
      <Seo title="Mi perfil · MEDIAZION" description="Gestiona tu perfil de mediador PRO: alias, bio, documentos y seguridad." />
      {/* Si prefieres navbar público, cambia el import de arriba y deja solo <Navbar/> */}
      {/* <NavbarPanel /> */}
      ...
      <main className="sr-container py-8" style={{minHeight:"calc(100vh - 160px)", background:"rgba(255,255,255,0.95)", borderRadius:16, margin:"24px 0"}}>
          ...
          <div className="sr-card">
            ...
            <div style={{display:"grid", gap:12, gridTemplateColumns:"1fr 1fr"}}>
              <div>
                ...
                {photoUrl && <div className="mt-2"><img src={photoUrl} alt="Avatar" style={{height:80, borderRadius:8}} /></div>}
              </div>
              <div>
                ...
                {cvUrl && <div className="mt-2"><a className="sr-btn-secondary" href={cvUrl} target="_blank" rel="noopener noreferrer">Ver CV</a></div>}
              </div>
            </div>
          </div>
        ...
        <section className="sr-card" style={{maxWidth:900, margin:"16px auto"}}>
