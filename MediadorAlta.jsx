...
const PRICE_ID  = import.meta.env.VITE_STRIPE_PRICE_ID; // price_xxx (49,90 €/mes)
...
export default function MediadorAlta(){
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // No vaciamos email para poder usarlo en la suscripción
    } catch (err) {
      ...
      <form onSubmit={onSubmit} className="sr-card mt-6" style={{maxWidth: 720}}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input required name="nombre" value={form.nombre} onChange={onChange}
              className="w-full border rounded-md px-3 py-2" placeholder="Nombre completo" />
          </div>
          <div>
            <label className="sr-p block mb-1">Email</label>
            <input required type="email" name="email" value={form.email} onChange={onChange}
              className="w-full border rounded-md px-3 py-2" placeholder="mediador@correo.com" />
          </div>
        </div>
