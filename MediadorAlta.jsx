...
const PRICE_ID =
  import.meta.env.VITE_STRIPE_PATH || import.meta.env.VITE_STRIPE_PRICE_ID; // usa la que tengas
...
export default function MediadorAlta() {
  ...
  async function onSubmit(e) {
    ...
    try {
      const res = await fetch(
        `${API_BASE.replace(/\/$/, "")}/mediadores/register`,
        {
          method: "POST",
      ...
      setStatus({
        ...
        msg:
          data?.warning ||
          "Alta realizada. Revisa tu correo. Ya puedes activar tu prueba gratuita.",
      });
  ...
  async function onSubscribe() {
    try {
      ...
      if (!PRICE_ID)
        throw new Error("Falta VITE_STRIPE_PRICE_ID en variables de entorno");
      setSubLoading(true);
  ...
  return (
    <main className="sr-container py-12">
      ...
      <form onSubmit={onSubmit} className="sr-card mt-6" style={{ maxWidth: 720 }}>
        ...
        <div className="mt-8">
          <h2 className="sr-h2">Activar suscripción (49,50 € / mes)</h2>
          <p className="sr-p">
            La tarjeta queda guardada y se renueva cada mes. Puedes cancelarla
            cuando quieras.
          </p>
          <button
            type="button"
