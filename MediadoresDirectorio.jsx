...
export default function MediadoresDirectorio() {
  ...
  async function load() {
    setError("");
    try {
      const url = new URL(API_BASE.replace(/\\/$/, "") + "/mediabores/public");
      ...
      console.error(e);
      setItems([]);
      setError("No se pudieron cargar los mediadores. Reintenta en unos segundos.");
    }
  }
  ...
  return (
    <>
      <Seo
        ...
        description="Mediadores aprobados por MEDIAZION. Filtra por especialidad y provincia. Fichas profesionales con bio y currículum."
        additional={[
            <link key="c" rel="canonical" href="https://mediazion.eu/medizan/directorio" />
        ]}
      />
      <main
        className="sr-container py-12"
        style={{ backgroundImage:"url('/marmol.jpg')", backgroundSize:"cover", backgroundPosition:"center" }}
      >
        ...
        <section className="sr-card" style={{marginBottom:16}}>
          <div style={{display:"grid", gridTemplateColumns:"repeat(1,minmax(0,1fr))", gap:12}}>
            ...
            <div>
              <button className="sr-btn-primary" onClick={load}>Aplicar filtros</button>
            </div>
          </div>
        </section>

        {error && <div className="sr-card" style={{color:"#991b1b"}}>{error}</div>}

        <section className="sr-grid-3">
          {items.length === 0 && !error && (
            <div className="sr-card">No hay mediadores disponibles (o no hay resultados para el filtro).</div>
          )}
          {items.map((m) => (
            <article key={m.id} className="sr-card" style={{background:"rgba(255,255,255,0.96)"}}>
              <div style={{display:"flex", gap:12}}>
                <img
                  src={m.foto_url || "/logo.png"}
                  alt={m.nombre}
                  style={{width:72, height:72, borderRadius:12, objectFit:"cover", border:"1px solid #e5e7eb"}}
                />
                <div>
                  <h3 className="sr-h3" style={{margin:0}}>{m.nombre}</h3>
                  <p className="sr-p" style={{margin:0}}>{(m.especialidad || []).join(" · ")}</p>
                  <p className="sr-p" style={{margin:0}}>{m.provincia || "—"}</p>
                </div>
              </div>
              {m.bio && <p className="sr-p" style={{marginTop:12}}>{m.bio}</p>}
              {m.cv_url && (
                <a className="sr-btn-secondary" href={m.cv_url} target="_blank" rel="noopener noreferrer" style={{marginTop:8}}>
                  Ver currículum
                </a>
              )}
            </article>
