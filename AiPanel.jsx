...
const BEARER = "ok"; // gate mínimo del backend de IA
...
export default function AiPanel() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "¡Hola! Soy tu asistente de mediación. Escribe a la derecha o pulsa un preset; luego pulsa “Generar” (o deja que el preset auto-envíe)." }
  ]);
    ...
    <>
      ...
      <main
        ...
        style={{
          ...
          borderRadius: 16, marginTop: 24, marginBottom: 24, backdropFilter: "blur(2px)"
        }}
      >
        {/* Header */}
              ...
              <div className="grid gap-3 mt-4">
                <div className="grid md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input className="sr-input" value={exportForm.case_no}
                      onChange={e => setExportForm(s => ({ ...s, case_no: e.target.value }))} placeholder="MED-2025-001" />
                  </div>
                  <div>
                    <label className="sr-label">Fecha</label>
                    <input className="sr-input" type="date" value={exportForm.date_iso}
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input className="sr-input" value={exportForm.mediator_alias}
                      onChange={e => setExportForm(s => ({ ...s, mediator_alias: e.target.value }))} />
                  </div>
                  <div>
                    <label className="sr-label">Partes</label>
                    <input className="sr-input" value={exportForm.parties}
                      onChange={e => setExportForm(s => ({ ...s, parties: e.target.value }))} placeholder="Parte A; Parte B" />
                  </div>
                </div>

                <div>
                  <label className="sr-label">Resumen / Desarrollo</label>
                  <textarea className="sr-input min-h-[120px]" value={exportForm.summary}
                    onChange={e => setExportForm(s => ({ ...s, summary: e.target.value }))} />
                </div>

                <div>
                  <label className="sr-label">Acuerdos alcanzados</label>
                  <textarea className="sr-input min-h-[100px]" value={exportForm.agreements}
                    onChange={e => setExportForm(s => ({ ...s, agreements: e.target.value }))} />
                </div>

                <div className="flex items-center gap-3">
                  <input id="confid" type="checkbox" checked={exportForm.confidentiality}
                ...
                <div>
                  ...
                  <input className="sr-input" value={exportForm.logo_url}
                    onChange={e => setExportForm(s => ({ ...s, logo_url: e.target.value }))} placeholder={LOGO_URL_DEFAULT} />
                </div>
              </div>
