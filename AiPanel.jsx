// src/pages/AiPanel.jsx — Asistente IA PRO (textarea visible, presets auto-enviar, upload doc, guardar sesión, export DOCX)
...
export default function AiPanel() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "¡Hola! Soy tu asistente de mediación. Escribe en el cuadro de la derecha (o debajo si usas móvil) o pulsa un preset para enviar directamente." }
  ]);
  ...
  function handleClearConversation() {
    setMessages([{ role: "assistant", content: "Conversación vaciada. Escribe en el cuadro (derecha o abajo) y pulsa “Generar”." }]);
    setErrorMsg("");
    setCopied(false);
  }
  ...
  async function handleSaveSession() {
    try {
      ...
      const r = await fetch("/api/ai/session/save", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + BEARER },
        body: JSON.stringify({ email, title: "Sesión de mediación", messages })
      });
      const data = await r.json().catch(() => ({}));
      if (!r.ok || !data?.ok) throw new Error(data?.detail || data?.message || "No se pudo guardar la sesión");
  ...
  async function handleFilePick(e) {
    ...
    try {
      ...
      setDocUrl(data.url); // p.ej. /uploads/archivo.pdf
      setUseDoc(true);
    } catch (e) {
  ...
  async function handleExport() {
    try {
      ...
      const data = await r.json().catch(() => ({}));
      if (!r.ok || !data?.ok || !data?.url) throw new Error(data?.detail || "No se pudo generar el DOCX");
  ...
  return (
    <>
      <Seo
        ...
        description="Asistente de redacción de actas, resúmenes y comunicaciones. Soporte de documentos y exportación a DOCX."
      ...
      >
        ...
        <div className="sr-card mb-4">
          <div className="flex flex-wrap gap-2">
            {PRESETS.map((p, i) => (
              <button
                key={i}
                className="px-3 py-1.5 rounded-full bg-sky-50 text-sky-800 border border-zinc-200 hover:bg-sky-100 transition"
        ...
        {/* Conversación + Editor (apilados en móvil, lado a lado en pantallas grandes) */}
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 w-[95%] max-w-2xl">
              ...
              <div className="grid gap-3 mt-4">
                ...
                <div>
                  ...
                  <textarea className="sr-input min-h-[120px]" value={exportForm.summary} onChange={e => setExportForm(s => ({ ...s, summary: e.target.value }))} />
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button className="sr-btn-primary" onClick={handleExport}>Generar DOCX</button>
